...
const nodedata = [
	{name:"INPUT",id:"n2",
		...
		body:[
			"<span class=uibase><input type=number></span>"
		]
	},
	{name:"node1",id:"n1",
		input:[
...
setjoint(id1,id2) {
	...
	const j1 = id1.split("_")
	const j2 = id2.split("_")
	if(id1==id2) {
...
addnode(param,pos) {
	...
	let str = []
	str.push(`<div class=head>${param.name}</div>`)
	str.push(`<div class=body>`)
	if(param.input) {
		str.push(`<div class=left>`)
		param.input.forEach(i=>{
			str.push(`<div><span class=joint id=${param.id}_i_${i.id}>●</span>${i.name}</div>`)
		})
		str.push(`</div>`)
	}
	str.push(`<div class=mid>${param.body.join("<br/>")}</div>`)
	if(param.output) {
		str.push(`<div class=right>`)
		param.output.forEach(i=>{
			str.push(`<div>${i.name}<span class=joint id=${param.id}_o_${i.id}>●</span></div>`)
		})
		str.push(`</div>`)
	}
	str.push(`</div>`)
...
//		console.log(o[0]+"/"+o[1])
	...
	this.f_startdrag = (ev)=> {
		const em = evm(ev) 
		if(document.elementFromPoint(em[0], em[1]).classList.contains("mdg")) this.cmenu.hide()
		if(!drag) return 
		dlog("dstart")
		ev.preventDefault()
	}
